...
"""
網站建置腳本
- 為 reports/*.html 加上導覽列（最新報表 / 歷史報表）
- 產生 index.html（= 最新一期報表的完整拷貝）
- 產生 history.html（掃描 reports/ 目錄，由新到舊列出所有日期）

用法：
    python3 build_site.py
執行後可重複執行；不會刪除 reports/ 裡任何既有檔案。
"""
import re
import glob
import os
...
def inject_nav(html, latest_href, history_href, active):
    # 插入 nav CSS（在 </style> 前）
    if "</style>" in html:
        html = html.replace("</style>", NAV_CSS + "</style>", 1)
    # 插入 nav bar（在 <div class="wrap"> 之後）
    nav_html = (
        '\n<nav class="site-nav">'
...
def main():
    ...
    dated_files.sort(key=lambda x: x[0])  # 舊到新

    if not dated_files:
        print("reports/ 內沒有找到 YYYY-MM-DD.html 格式的檔案，中止。")
        return

    # 1) 幫每個 reports/*.html 加上導覽列（若尚未加過）
    for d, f in dated_files:
        ...
        if 'class="site-nav"' not in html:
            ...
            with open(f, "w", encoding="utf-8") as fh:
                fh.write(html)
            print(f"已加上導覽列: reports/{os.path.basename(f)}")
        else:
            print(f"已存在導覽列，略過: reports/{os.path.basename(f)}")

    # 2) 產生 index.html = 最新一期報表的完整拷貝
    latest_date, latest_file = dated_files[-1]
    with open(latest_file, "r", encoding="utf-8") as fh:
        latest_html = fh.read()
    # index.html 位於根目錄，路徑要改成不含 ../
